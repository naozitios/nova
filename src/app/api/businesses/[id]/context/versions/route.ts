...
import { listVersions } from '@/core/business-context/service'
...
// GET /api/businesses/{id}/context/versions — list all profile versions
export async function GET(
  ...
  const result = await listVersions(repo, businessId, wsResult.workspaceId)
  if (!result.ok) {
    return errorResponse(500, result.error.code, result.error.message)
  }

  const versions = result.data.items.map((v) => ({
    id: v.id,
  ...
  return jsonResponse({ versions, total: result.data.total })
}
