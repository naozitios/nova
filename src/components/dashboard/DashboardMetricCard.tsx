...
export function DashboardMetricCard({ metric }: DashboardMetricCardProps) {
  ...
  const content = (
    <div className="group h-full rounded-3xl border border-stone-100 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:border-stone-200 hover:shadow-md">
      <div className="mb-3 flex items-center justify-between gap-4">
        <span className="text-sm font-medium text-stone-500">{metric.label}</span>
        <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-br from-[#FEF3E2] to-[#FFDAB9]">
          <Icon className="h-4 w-4 text-[#E55A3C]" />
        </span>
      </div>
